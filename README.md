...
- ***Project 5-6 Step 9 (pg 212)*** - If you are prompted to run the `systemctl daemon-reload` command after executing the `mount` command in this step, please do so before continuing to the next step. This is because Fedora now converts entries in /etc/fstab to Systemd mount units (discussed later in Chapter 8), but this conversion is performed only during system startup.
