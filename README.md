...
- ***Project 5-6 Step 9 (pg 212)*** - Fedora now converts entries in /etc/fstab to Systemd mount units (discussed later in Chapter 8), but this conversion is performed only during system startup. As a result, after executing the `mount` command in this step run the `systemctl daemon-reload` command to manually perform this conversion. 
